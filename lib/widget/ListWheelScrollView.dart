...
/// A delegate that supplies children for [ListWheelScrollView2].
///
/// [ListWheelScrollView2] lazily constructs its children during layout to avoid
/// creating more children than are visible through the [Viewport]. This
/// delegate is responsible for providing children to [ListWheelScrollView2]
/// during that stage.
///
/// See also:
///
...
///    a builder callback.
abstract class ListWheelChildDelegate2 {
  ...
  bool shouldRebuild(covariant ListWheelChildDelegate2 oldDelegate);
}

/// A delegate that supplies children for [ListWheelScrollView2] using an
/// explicit list.
///
/// [ListWheelScrollView2] lazily constructs its children to avoid creating more
...
/// conditions.
class ListWheelChildListDelegate2 extends ListWheelChildDelegate2 {
  /// Constructs the delegate from a concrete list of children.
  ListWheelChildListDelegate2({@required this.children}) : assert(children != null);
  ...
  @override
  bool shouldRebuild(covariant ListWheelChildListDelegate2 oldDelegate) {
    return children != oldDelegate.children;
  }
}

/// A delegate that supplies infinite children for [ListWheelScrollView2] by
...
/// demand). For example, the body of a dialog box might fit both of these
/// conditions.
class ListWheelChildLoopingListDelegate2 extends ListWheelChildDelegate2 {
  /// Constructs the delegate from a concrete list of children.
  ListWheelChildLoopingListDelegate2({@required this.children}) : assert(children != null);
  ...
  @override
  bool shouldRebuild(covariant ListWheelChildLoopingListDelegate2 oldDelegate) {
    return children != oldDelegate.children;
  }
}

/// A delegate that supplies children for [ListWheelScrollView2] using a builder
/// callback.
///
/// [ListWheelScrollView2] lazily constructs its children to avoid creating more
/// children than are visible through the [Viewport]. This delegate provides
...
class ListWheelChildBuilderDelegate2 extends ListWheelChildDelegate2 {
  /// Constructs the delegate from a builder callback.
  ListWheelChildBuilderDelegate2({
  ...
  @override
  bool shouldRebuild(covariant ListWheelChildBuilderDelegate2 oldDelegate) {
...
/// See also:
///
///  * [ListWheelScrollView2], a scrollable view widget with fixed size items
///    that this widget controls.
///  * [FixedExtentMetrics2], the `metrics` property exposed by
///    [ScrollNotification] from [ListWheelScrollView2] which can be used
///    to listen to the current item index on a push basis rather than polling
///    the [FixedExtentScrollController2].
class FixedExtentScrollController2 extends ScrollController {
  ...
  ///
  /// There are circumstances that this [FixedExtentScrollController2] can't know
  /// the current item. Reading [selectedItem] will throw an [AssertionError] in
  /// the following cases:
  ///
  /// 1. No scroll view is currently using this [FixedExtentScrollController2].
  /// 2. More than one scroll views using the same [FixedExtentScrollController2].
  ///
  ...
  int get selectedItem {
    ...
    'FixedExtentScrollController2.selectedItem cannot be accessed before a '
        'scroll view is built with it.',
    );
    assert(
    positions.length == 1,
    'The selectedItem property cannot be read when multiple scroll views are '
        'attached to the same FixedExtentScrollController2.',
    );
    final _FixedExtentScrollPosition2 position = this.position;
  ...
  Future<void> animateToItem(
    ...
    for (_FixedExtentScrollPosition2 position in positions) {
  ...
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition oldPosition) {
    return _FixedExtentScrollPosition2(
      physics: physics,
      context: context,
...
/// views such as [ListWheelScrollView2]s with a [FixedExtentScrollController2] and
...
class FixedExtentMetrics2 extends FixedScrollMetrics {
  /// Creates an immutable snapshot of values associated with a
  /// [ListWheelScrollView2].
  FixedExtentMetrics2({
...
class _FixedExtentScrollPosition2 extends ScrollPositionWithSingleContext implements FixedExtentMetrics2 {
  _FixedExtentScrollPosition2({
  ...
  context is _FixedExtentScrollableState2,
  'FixedExtentScrollController2 can only be used with ListWheelScrollView2s'
  ),
        super(
        physics: physics,
        context: context,
  ...
  static double _getItemExtentFromScrollContext(ScrollContext context) {
    final _FixedExtentScrollableState2 scrollable = context;
...
/// size so it can pass it on ultimately to the [FixedExtentScrollController2].
class _FixedExtentScrollable2 extends Scrollable {
  const _FixedExtentScrollable2({
    Key key,
  ...
  @override
  _FixedExtentScrollableState2 createState() => _FixedExtentScrollableState2();
}

/// This [ScrollContext] is used by [_FixedExtentScrollPosition] to read the
/// prescribed [itemExtent].
class _FixedExtentScrollableState2 extends ScrollableState {
  double get itemExtent {
    // Downcast because only _FixedExtentScrollable can make _FixedExtentScrollableState.
    final _FixedExtentScrollable2 actualWidget = widget;
...
/// that item.
///
/// Must be used with a scrollable that uses a [FixedExtentScrollController2].
///
/// Defers back to the parent beyond the scroll extents.
class FixedExtentScrollPhysics2 extends ScrollPhysics {
  /// Creates a scroll physics that always lands on items.
  const FixedExtentScrollPhysics2({ ScrollPhysics parent }) : super(parent: parent);

  @override
  FixedExtentScrollPhysics2 applyTo(ScrollPhysics ancestor) {
    return FixedExtentScrollPhysics2(parent: buildParent(ancestor));
  }

  @override
  Simulation createBallisticSimulation(ScrollMetrics position, double velocity) {
    assert(
    position is _FixedExtentScrollPosition2,
    'FixedExtentScrollPhysics2 can only be used with Scrollables that uses '
        'the FixedExtentScrollController2'
    );

    final _FixedExtentScrollPosition2 metrics = position;
...
/// a plane.
class ListWheelScrollView2 extends StatefulWidget {
  ...
  ListWheelScrollView2({
    ...
    @required List<Widget> children,
        ...
        childDelegate = ListWheelChildListDelegate2(children: children),
        super(key: key);

  /// Constructs a list in which children are scrolled a wheel. Its children
  /// are managed by a delegate and are lazily built during layout.
  const ListWheelScrollView2.useDelegate({
    Key key,
    this.controller,
    this.physics,
    this.diameterRatio = RenderListWheelViewport.defaultDiameterRatio,
  ...
  /// Typically a [FixedExtentScrollController2] used to control the current item.
  ///
  /// A [FixedExtentScrollController2] can be used to read the currently
  /// selected/centered child item and can be used to change the current item.
  ///
  /// If none is provided, a new [FixedExtentScrollController2] is implicitly
  /// created.
  ///
  /// If a [ScrollController] is used instead of [FixedExtentScrollController2],
  /// [ScrollNotification.metrics] will no longer provide [FixedExtentMetrics2]
  ...
  final ListWheelChildDelegate2 childDelegate;

  @override
  _ListWheelScrollView2State createState() => _ListWheelScrollView2State();
}

class _ListWheelScrollView2State extends State<ListWheelScrollView2> {
  ...
  void initState() {
    super.initState();
    scrollController = widget.controller ?? FixedExtentScrollController2();
    if (widget.controller is FixedExtentScrollController2) {
      final FixedExtentScrollController2 controller = widget.controller;
      _lastReportedItemIndex = controller.initialItem;
    }
  }

  @override
  void didUpdateWidget(ListWheelScrollView2 oldWidget) {
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (ScrollNotification notification) {
        if (notification.depth == 0
            ...
            && notification.metrics is FixedExtentMetrics2) {
          final FixedExtentMetrics2 metrics = notification.metrics;
          ...
          if (currentItemIndex != _lastReportedItemIndex) {
            ...
            print("打印   build 000   ${trueIndex}");
          }
        }
        return false;
      },
      child: _FixedExtentScrollable2(
...
class ListWheelElement extends RenderObjectElement implements ListWheelChildManager {
  ...
  void update(ListWheelViewport newWidget) {
    ...
    final ListWheelChildDelegate2 newDelegate = newWidget.childDelegate;
    final ListWheelChildDelegate2 oldDelegate = oldWidget.childDelegate;
...
///
/// Typically used with [ListWheelScrollView2], this viewport is similar to
...
/// See also:
///
///  * [ListWheelScrollView2], widget that combines this viewport with a scrollable.
